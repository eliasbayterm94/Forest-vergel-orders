...
/**
 * POST /fermentation-types-create  (admin)
 * Body: { name, kind? }
 * Idempotente: si existe (case-insensitive), reactiva y/o actualiza kind.
 */
exports.handler = requireAuth(['admin'], async (event) => {
  ...
  const { data: existing, error: lookupErr } = await sb
    .from('fermentation_types').select('id, name, kind, active').eq('name', name).maybeSingle();
  ...
  if (existing) {
    ...
    if (Object.keys(upd).length > 0) {
      const { data: updated } = await sb
        .from('fermentation_types').update(upd).eq('id', existing.id)
        .select('id, name, kind, active').single();
      return ok({ fermentation_type: updated, created: false });
    }
    return ok({ fermentation_type: existing, created: false });
  }

  const { data, error } = await sb
    .from('fermentation_types').insert({ name, kind }).select('id, name, kind, active').single();
  if (error) return serverErr('Insert failed', error.message);
  return created({ fermentation_type: data, created: true });
});
