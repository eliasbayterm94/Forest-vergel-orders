...
/**
 * POST /drying-types-update  (admin)
 * Body: { id, fields: { name?, kind?, active? } }
 *
 * Para renombrar, recategorizar o desactivar un tipo. No borramos
 * (los lotes históricos pueden tener el nombre referenciado en
 * production_lots.drying_locations).
 */
...
exports.handler = requireAuth(['admin'], async (event) => {
  ...
  if (update.kind != null) {
    update.kind = String(update.kind).trim() || null;
  }
  ...
  const { data, error } = await sb
    .from('drying_types').update(update).eq('id', id)
    .select('id, name, kind, active').maybeSingle();
  if (error) return serverErr('Update failed', error.message);
  if (!data) return notFound('Drying type not found');
  return ok({ drying_type: data });
});
