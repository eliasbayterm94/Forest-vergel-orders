...
/**
 * POST /drying-types-create  (admin)
 * Body: { name, kind? }
 *
 * Idempotente: si ya existe (case-insensitive), devuelve el existente.
...
exports.handler = requireAuth(['admin'], async (event) => {
  ...
  const { data: existing, error: lookupErr } = await sb
    .from('drying_types').select('id, name, kind, active').eq('name', name).maybeSingle();
  if (lookupErr) return serverErr('Lookup failed', lookupErr.message);
  if (existing) {
    // Si estaba inactivo, lo reactivamos. También sincroniza kind si vino.
    const upd = {};
    if (!existing.active) upd.active = true;
    if (kind != null && kind !== existing.kind) upd.kind = kind;
    if (Object.keys(upd).length > 0) {
      const { data: updated } = await sb
        .from('drying_types').update(upd).eq('id', existing.id)
        .select('id, name, kind, active').single();
      return ok({ drying_type: updated, created: false });
    }
    return ok({ drying_type: existing, created: false });
  }

  const { data, error } = await sb
    .from('drying_types').insert({ name, kind }).select('id, name, kind, active').single();
  if (error) return serverErr('Insert failed', error.message);
  return created({ drying_type: data, created: true });
});
