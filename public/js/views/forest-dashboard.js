...
function assignedLotsDetail(lots) {
  ...
  // kg seco, kg verde y factor de rendimiento promedio ponderado para
  // cada Status). El factor ponderado pesa cada lote por sus kg seco:
  //    factor_avg = Σ(factor_i · kg_dried_i) / Σ(kg_dried_i)
  // (mismo cálculo que se usa al cerrar un bache con parciales).
  const totalDried = sorted.reduce((s, l) => s + Number(l.kg_dried_output || 0), 0);
  function weightedFactor(items) {
    let num = 0, den = 0;
    for (const l of items) {
      const f = Number(l.factor_rendimiento || 0);
      const w = Number(l.kg_dried_output || 0);
      if (f > 0 && w > 0) { num += f * w; den += w; }
    }
    return den > 0 ? Math.round((num / den) * 100) / 100 : null;
  }
  const totalFactor = weightedFactor(sorted);

  const STATE_ORDER = ['InFermentation', 'Drying', 'Resting', 'Ready', 'Delivered'];
  const byState = new Map();
  for (const l of sorted) {
    if (!byState.has(l.status)) byState.set(l.status, { count: 0, dried: 0, green: 0, items: [] });
  ...
  const tfoot = el('tfoot', {}, [el('tr', { class: 'border-t-2 border-ink-300 bg-cream' }, [
    ...
    el('td', { class: 'px-2 py-1.5 text-right font-mono font-semibold text-navy', text: totalFactor != null ? String(totalFactor) : '—' }),
    el('td', { class: 'px-2 py-1.5' }, []),
